...
//        self.readLb.font = FontWithName("DINPro-Regular", 10)
        ...
        self.readLb.font = UIFont.systemFont(ofSize: 10)
    ...
    func setHeadInfo(_ headInfo:Yuanzu){
        var headurl = headInfo.headImge
        let headurlArr = headurl.components(separatedBy: "?")
        if headurlArr.count > 0{
            headurl = headurlArr[0]
        }
        let Url = URL.init(string: headurl)
        self.headImage.sd_setImage(with: Url, completed: nil)
    ...
    func setModel(_ model:JianShuModel){
        ...
        let newYear = dateToStr(dateFormat: "YYYY", date: Date())
        ...
        if model.wrap!.count > 0 {
            ...
            let xdurl = URL.init(string: wrapArr[0])
            
            self.wrapImg.sd_setImage(with: xdurl, completed: nil)
