...
class JianShuRequestModel: NSObject {
    ...
    static func jianshuRequestDataWithPage(_ page:Int,_ itemWidth:Float, _ headBlock:HeadBlock, _ datalistBack:DataListBack){
        
        //初始化model数组
        var dataArr = [JianShuModel]()
        do {
            //作者ID
            let authorID = "782695e147fa"
            //获取HTML源码
            let htmlurl = "https://www.jianshu.com/u/\(authorID)?page=\(page)"
            var str = try String(contentsOf: URL.init(string: htmlurl)!,encoding: .utf8)
            //剔除换行符和空格
            ...
            if page == 1 {
                ...
                //头部高度计算
                let headerH = 10 + 60 + 5 + 12 + 8 + GETSTRHEIGHT(fontSize: 12, width: CGFloat(kScreenWidth - (10 + 60 + 15 + 10)) , words: intro) + 10 + 1
