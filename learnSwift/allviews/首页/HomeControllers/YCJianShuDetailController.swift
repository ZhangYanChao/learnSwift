//
//  YCJianShuDetailController.swift
//  learnSwift
//
...
class YCJianShuDetailController: BaseViewController,WKUIDelegate,WKNavigationDelegate,WKScriptMessageHandler,UIScrollViewDelegate{
    
    override func viewDidLoad() {
        super.viewDidLoad()
        self.navigationItem.title = "详情"
        //设置左侧返回键
        let barItem = UIBarButtonItem.init(title: "返回", style: .plain, target: self, action: #selector(backToLastVC))
        ...
        //加载“菊花”
        self.topIndicator = UIActivityIndicatorView.init(frame: CGRect(x: (kScreenWidth - 100) / 2.0, y: (kScreenHeight - 100) / 2.0, width: 100, height: 100))
        self.topIndicator?.style = .whiteLarge
    ...
    var webView:WKWebView = {
        ...
        let webView =  WKWebView.init(frame: CGRect(x: 0, y: 0, width: kScreenWidth, height: kScreenHeight - 64 - (IsFullScreen ? 34 : 0)), configuration:configuration)
        return webView
    }()
